...
function seed(root: string) {
  ...
  writeFileSync(join(root, 'src', 'routes', 'docs.tsx'), '// docs');
  writeFileSync(join(root, 'src', 'routes', 'example.tsx'), '// example');
  writeFileSync(join(root, 'src', 'routes', 'watch.tsx'), '// watch');
...
describe('scripts/strip-example.mjs', () => {
  ...
  it('deletes example route files but keeps the router root', () => {
    run(tmp);
    for (const name of ['docs.tsx', 'example.tsx', 'watch.tsx']) {
      expect(existsSync(join(tmp, 'src', 'routes', name))).toBe(false);
    }
    expect(existsSync(join(tmp, 'src', 'routes', 'index.tsx'))).toBe(true);
  });

  it('rewrites src/routes/index.tsx to render the new Home page', () => {
    run(tmp);
    const contents = readFileSync(join(tmp, 'src', 'routes', 'index.tsx'), 'utf8');
  ...
  it('--dry-run prints changes without writing', () => {
    ...
    expect(existsSync(join(tmp, 'src', 'routes', 'docs.tsx'))).toBe(true);
