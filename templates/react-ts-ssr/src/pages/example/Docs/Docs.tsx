...
const SECTIONS: Section[] = [
  {
    ...
    summary:
      'A modern React app needs routing, forms, data fetching, mocked APIs, three tiers of tests, linting, formatting, git hooks, dependency updates, and a release pipeline before you write a single line of feature code. This boilerplate wires all of it once, uses one file for network fixtures across every environment, and keeps configs fresh through Renovate so your project never fossilizes at scaffold time. It is opinionated on purpose: routing + SSR is Vike (with vike-react), server state is TanStack Query, forms are react-hook-form + zod, tests are Vitest + Storybook + Playwright, and mocks are MSW. Fewer decisions, more shipping.',
...
Type-safe end to end: zod validates env, search params, and forms; Vike + vike-react types routes and page config.
...
# re-export from src/components/atoms/index.ts`,
  ...
  {
    id: 'routing',
    title: 'File-based routing + SSR (Vike)',
    summary:
      'Vike. Drop a folder in pages/ with a +Page.tsx file and it becomes a route. Add +title.ts and +description.ts for per-route <head> meta that appears in prerendered HTML so social crawlers see it. Every route is prerendered to static HTML at build time (prerender: true in pages/+config.ts), so deploys stay on plain static hosting - no server required. Read search params on the server via usePageContext().urlParsed.search.',
    usage: `// pages/watch/+Page.tsx
import { usePageContext } from 'vike-react/usePageContext';

export default function Page() {
  const ctx = usePageContext();
  const v = (ctx.urlParsed.search as { v?: string }).v ?? null;
  return <Watch videoId={v} />;
}

// pages/watch/+title.ts
export const title = 'Watch · react-app-boilerplate';`,
    ...
    learnMore: [
      ...
      {
        label: 'vike-react + config files',
        href: 'https://vike.dev/vike-react',
      },
      {
        label: 'Prerender / SSG',
...
feat, fix, docs, style, refactor, perf, test, build, ci, chore, revert`,
  ...
  },
  {
    ...
    title: 'Deploying',
    summary:
      'Any static host works. Vite + Vike prerender to dist/client/, emitting real HTML per route (index.html, docs/index.html, example/index.html, watch/index.html). Social crawlers get populated <head> meta on every URL. No server / no SPA fallback is required - each route is a file. Only VITE_* env vars are baked into the client bundle at build time. MSW does not run in production, so your prod host needs a real backend or a proxy.',
...
# Docker (nginx): see docs/guides/docker.md in the parent repo.`,
    files: ['vite.config.ts', 'pages/+config.ts', 'pages/+Head.tsx', 'src/lib/env.ts'],
    learnMore: [
      ...
      <article className="flex min-w-0 flex-1 flex-col gap-10">
        ...
        <section id="conventions" className="flex scroll-mt-20 flex-col gap-4">
          ...
          <p className="max-w-2xl text-muted">
            ...
            Vike page directory (+Page.tsx + +title.ts). The generated tests include an axe
            assertion so the a11y floor never drops. Follow the same shape for anything you add by
            hand, or let the generator do it for you.
          </p>
